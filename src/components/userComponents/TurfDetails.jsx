...
const TurfDetails = () => {
  ...
  const handleBookingConfirm = async () => {
    ...
    //razorpay
    const bookedSlot=turfDetails.slots.find((option)=>option.type==selectOption )
    console.log(bookedSlot)
    const res=await PaymentOrderAPi(bookedSlot.price)
    console.log(res)
    ...
    var options = {
      ...
      "currency": currency,
      "name": notes.name, //your business name
      ...
      "prefill": { //We recommend using the prefill parameter to auto-fill customer's contact information especially their phone number
          ...
          "email": notes.emailId,
          "contact": notes.phone //Provide the customer's phone number for better conversion rates 
      },
      "notes": {
      ...
      handler:async function(response){
        try {
          ...
          console.log(err)
          toast.error('payment done but booking failed , please contact admin')
  ...
  const bookingForSelectedDates = myBookings.filter((b) => {
    ...
    return bookingDay == inputdate && b.slot == selectOption;
  });
  ...
  const bookingForEndDate = myBookings.filter((b) => {
    ...
    const startingbooking = new Date(
      `${inputdate} ${selectedStartTime}`
    ).getTime();
        ...
        <div className="md:flex gap-10">
          ...
          <div className="md:w-1/2 rounded-3xl overflow-hidden shadow-xl border border-white/40 bg-white/80">
            <div className="relative group">
              ...
              <div
                className="absolute top-4 left-4 bg-green-600/90 text-white px-4 py-1
          ...
          <div className="flex flex-wrap justify-center gap-4 mt-6">
            {slots.map((time) => {
              const isbooked = bookingForSelectedDates.some((b) => {
                const bookStart = new Date(b.startTime).toLocaleTimeString(
                  "en-US",
                  {
                    hour: "numeric",
                    minute: "2-digit",
                ...
                const bookedEnd = new Date(b.endTime).toLocaleTimeString(
                  "en-US",
                  {
                    hour: "numeric",
                    minute: "2-digit",
              ...
              {slots
                ...
                .map((time) => {
                  ...
                  const isBooked = bookingForSelectedDates.some((b) => {
                    ...
                    const bookEnd = new Date(b.endTime).toLocaleTimeString(
                      "en-US",
                      {
                        hour: "numeric",
                        minute: "2-digit",
                    ...
                    return (
                      selectedIndex < endIndex && startIndex < currentIndex
                    );
                  });
