...
import { blockuserApi, getAllownerApi } from '../../server/allApi'
...
const AdminAllowner = () => {
  ...
  const getOwners=async()=>{
    try {
      const res=await getAllownerApi()
      setShimmer(false)
      if(res.status==200){
    ...
    <div className="mx-6 mt-6">
      <div className="bg-gradient-to-r from-emerald-600 to-green-700 p-8 rounded-3xl shadow-lg">
        ...
        <p className="text-emerald-100 mt-1 font-['figtree']">
          Manage, review and control turf owners
        </p>
      </div>
    </div>
                ...
                >
                  ...
                  <td className="px-6 py-4">
                    <div className="flex justify-center gap-2">
                      <Link to={`/admin/ownerview/${owner._id}`}>
                      ...
                      <button
                        onClick={() =>
                          handleBlock(owner._id, owner.isBlocked ? false : true)
                        }
